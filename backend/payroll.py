...
### ⛔ 프로토타입 범위 — 일부러 넣지 않은 것
아래는 실제 급여와의 차이를 만드는 항목이고, 시연 범위 밖이라 **의도적으로 뺐다.**
숫자를 손보기 전에 이 목록을 먼저 읽을 것.

  1. **주휴수당** — 근로기준법 제55조. 주 15시간 이상이면 유급휴일이 발생한다.
     여기서는 월 근로시간을 `21.7일 x 8시간 = 173.6시간`으로 잡지만, 최저임금
     월 환산에 쓰는 기준은 주휴 8시간을 더한 **209시간**(주 48시간 x 4.345주)이다.
     그래서 이 화면의 예상 급여는 실제보다 **약 17% 적다.**
  2. **4대보험 공제와 소득세** — 국민연금 4.5%, 건강보험과 장기요양, 고용보험,
     그리고 근로소득세. E-9 은 단일세율 19% 과세특례를 고를 수도 있다.
  3. **연장·야간 가산의 정확한 요율** — 여기서는 연장 1.5배만 단순 적용한다.

1번은 숫자를 낮추고 2번은 높인다. 서로 상쇄되지 않으므로 **둘 다 빠졌다는 사실을
화면에 적어 두는 것**으로 대신한다. 실서비스에서는 회사 급여명세를 그대로 읽는다.
...
def month_summary(worker_name: str, live_records: list | None = None,
    ...
    return {
        ...
        "authority": "mock",
        "notice": ("주휴수당과 4대보험·세금 공제를 넣지 않은 값입니다. "
                   "시연용 어림값이며 실제 급여는 회사 급여명세가 기준입니다."),
    }
